    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-2">
                ...
                @endif
                <a href="{{ auth()->user()->isAdmin() ? route('admin.tickets') : route('tickets.index') }}" class="text-sm text-gray-600 hover:text-gray-900">&larr; Back</a>
            </div>
        </div>
    </x-slot>
                ...
                <div class="p-6">
                    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Submitted on {{ $ticket->created_at->format('F d, Y \a\t g:i A') }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 pb-6 border-b border-gray-200">
                        <div>
                            <span class="text-xs font-medium text-gray-500 uppercase">Campus</span>
                            <p class="text-sm text-gray-900 mt-1">{{ $ticket->campus->name }}</p>
                        </div>
                        <div>
                            <span class="text-xs font-medium text-gray-500 uppercase">Category</span>
                            <p class="text-sm text-gray-900 mt-1">{{ $ticket->category->name }}</p>
                        </div>
                        <div>
                            <span class="text-xs font-medium text-gray-500 uppercase">Department</span>
                            <p class="text-sm text-gray-900 mt-1">{{ $ticket->category->department }}</p>
                        </div>
                        <div>
                            <span class="text-xs font-medium text-gray-500 uppercase">Assigned To</span>
                            <p class="text-sm text-gray-900 mt-1">{{ $ticket->assignee?->name ?? 'Unassigned' }}</p>
                        </div>
                    </div>

                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-700 mb-2">Description</h3>
                        <div class="bg-gray-50 rounded-lg p-4 text-sm text-gray-900 whitespace-pre-wrap">{{ $ticket->description }}</div>
                    </div>

                    @if($ticket->attachments->count() > 0)
                        <div class="mb-6">
                            <h3 class="text-sm font-medium text-gray-700 mb-2">Attachments ({{ $ticket->attachments->count() }})</h3>
                            <div class="flex flex-wrap gap-3">
                                @foreach($ticket->attachments as $attachment)
                                    <a href="{{ route('tickets.attachments.download', $attachment) }}" class="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 border border-gray-200">
                    ...
                    @if($ticket->resolution)
                        <div class="mb-6">
                            <h3 class="text-sm font-medium text-gray-700 mb-2">Resolution</h3>
                            <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-sm text-green-900 whitespace-pre-wrap">{{ $ticket->resolution }}</div>
                            @if($ticket->resolved_at)
                                <p class="text-xs text-green-600 mt-2">Resolved on {{ $ticket->resolved_at->format('F d, Y \a\t g:i A') }}</p>
                            @endif
                        </div>
                    @endif
                    ...
                    @if(!auth()->user()->isStudent())
                        <div class="border-t border-gray-200 pt-6">
                            <h3 class="text-sm font-medium text-gray-700 mb-4">Update Ticket</h3>
                            <form action="{{ route('tickets.update', $ticket) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label for="resolution" class="block text-sm font-medium text-gray-700">Resolution / Notes</label>
                                    <textarea name="resolution" id="resolution" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Add resolution details...">{{ $ticket->resolution }}</textarea>
                                </div>

                                <div class="mb-4">
                                    <label for="comment" class="block text-sm font-medium text-gray-700">Internal Comment</label>
                                    <textarea name="comment" id="comment" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Add a comment about this update..."></textarea>
                                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="flow-root">
                        <ul role="list" class="-mb-8">
                            ...
                            @empty
                                <li class="text-sm text-gray-500 text-center py-4">No activity yet</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
