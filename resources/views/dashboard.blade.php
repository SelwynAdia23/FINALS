    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->user()->isAdmin())
                <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition">
                    Admin Panel
                </a>
            @endif
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                    <div class="text-sm font-medium text-gray-500">Total Concerns</div>
            ...
            @if($stats['urgent'] > 0)
                <div class="mb-6 bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 text-red-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                        <span class="text-red-800 font-medium">You have {{ $stats['urgent'] }} urgent concern(s) that need attention</span>
                    </div>
                </div>
            @endif

            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-semibold text-gray-900">Recent Concerns</h3>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @if($recentTickets->count() > 0)
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($recentTickets as $ticket)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{{ $ticket->subject }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $ticket->category->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $ticket->campus->code }}</td>
                    ...
                    <div class="p-4 border-t border-gray-200">
                        <a href="{{ route('tickets.index') }}" class="text-sm text-blue-600 hover:text-blue-800">View all concerns &rarr;</a>
                    </div>
                @else
                    <div class="p-8 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900">No concerns yet</h3>
                        <p class="mt-1 text-sm text-gray-500">Get started by submitting your first concern.</p>
                        <div class="mt-6">
